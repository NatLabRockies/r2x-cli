...
class CiPythonVersionTests(unittest.TestCase):
    ...
    def test_setup_action_preserves_requested_version_and_exports_abi(self):
        ...
        self.assertIn('resolved_query_ref="${requested_version}"', setup_script)
        self.assertIn('uv python find "${resolved_query_ref}"', setup_script)
        self.assertIn('resolved_query_ref="${requested_abi}"', setup_script)
